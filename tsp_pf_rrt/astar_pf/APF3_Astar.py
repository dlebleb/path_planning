...
#            ax,expanded_rects, eps_expanded_rects, full_geometric_path,stop_counter):
   ...
   # Move rectangles (ONLY x, y)
    ...
    q_old = q.copy()
    q[:] = q + direction * v_robot * dt #* (F_norm*0.0)
   
    # L = 1.0   # local horizon
    # q_target_local = q + L * direction * 0.3 # added 0.3 recently and it seems to be better
    ...
    if np.linalg.norm(q - q_goal) < tolerance: # if arrives at at a stop
        ...
        if np.all(q_goal == q_goal_final):
            ...
            q_goal = full_geometric_polyline[1+stop_counter]
